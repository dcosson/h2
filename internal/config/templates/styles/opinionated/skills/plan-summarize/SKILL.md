...
1. **Use narrative text for finding themes, not tables.** For each round, write 3-5 short paragraphs (1-2 sentences each), where each paragraph covers a distinct theme observed in that round's findings. Themes might include: cross-doc consistency gaps, API contract mismatches, test harness staleness, missing error handling, ERC registration divergence, etc. If fewer than 3 meaningfully different themes emerged, just cover the themes that exist — don't pad with filler. Each paragraph should name the theme, give a concrete example or two (citing specific docs), and note how many findings fell into that category. Example paragraph: "The dominant theme was **cross-document interface inconsistency** — 12 of 28 findings identified structs or enums in one doc that didn't match the canonical definition in a dependency doc (e.g., 05d's CapabilitySet missing fields defined in 02a §4.2, 07a's SchemaVersion semantics conflicting with 06a §8.1)." Bad: A table with columns "Theme | Count | Share".
