...
func (vt *VT) Resize(totalRows, cols, childRows int) {
	...
	// On width change, drop ScrollHistory entries. Each entry is a pre-rendered
	// ANSI string captured at the width that was current when OnScrollback
	// fired. Replaying it at the new width either truncates (narrower) or wraps
	// and collides with the row below (wider), producing the "duplicate lines
	// and blank lines in scrollback" symptom users see after a monitor unplug.
	// We have no way to re-render captured entries at the new width — only the
	// final ANSI bytes survived — so the only reflow-safe option is to drop
	// them. The active screen's content is regenerated by the child on SIGWINCH
	// so visible content (live view) is unaffected.
	if widthChanged {
		vt.ScrollHistory = nil
	}
