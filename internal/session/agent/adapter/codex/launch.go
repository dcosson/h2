...
func BuildLaunchConfig(cb otelserver.Callbacks) (adapter.LaunchConfig, *otelserver.OtelServer, error) {
	s, err := otelserver.New(cb)
	if err != nil {
		return adapter.LaunchConfig{}, nil, fmt.Errorf("create otel server: %w", err)
	}
