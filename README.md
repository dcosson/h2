...
Typing `ctrl + \` will take you to the Menu mode, where you can detach or quit (kill) the agent process. Typing `p` here will take you to Passthrough mode. Typing `f` forks the current session: the conversation history is copied to a new, independent agent (named like `fond-birch-fork1`, not part of the parent's pod) that starts in the background — the status bar shows "Forked to <name>" and you stay on the current session; switch to the fork with the agent navigator or `h2 attach` (also available as `h2 run [fork-name] --fork-from <agent>` or `h2 session fork <agent>`). Typing `a` opens the agent navigator: running agents grouped by pod (like `h2 list`), then lone agents by most recent activity, with stopped-but-resumable sessions at the end. Move with the arrow keys and hit Enter to switch — selecting a stopped session resumes it first, then switches over.
